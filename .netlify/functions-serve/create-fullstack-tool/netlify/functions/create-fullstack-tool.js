  ...
  "node_modules/@supabase/functions-js/dist/main/helper.js"(exports2) {
    ...
    var resolveFetch = (customFetch) => {
      ...
      if (customFetch) {
        ...
        _fetch = (...args) => Promise.resolve(`${"@supabase/node-fetch"}`).then((s) => __importStar2(require(s))).then(({ default: fetch3 }) => fetch3(...args));
      } else {
        _fetch = fetch;
      }
...
var require_PostgrestBuilder = __commonJS({
  "node_modules/@supabase/postgrest-js/dist/cjs/PostgrestBuilder.js"(exports2) {
    ...
    Object.defineProperty(exports2, "__esModule", { value: true });
    var tslib_1 = (init_tslib_es6(), __toCommonJS(tslib_es6_exports));
    var node_fetch_1 = tslib_1.__importDefault(require_lib3());
      ...
      constructor(endPoint, options) {
        ...
        this._resolveFetch = (customFetch) => {
          ...
          if (customFetch) {
            ...
            _fetch = (...args) => Promise.resolve(`${"@supabase/node-fetch"}`).then((s) => tslib_1.__importStar(require(s))).then(({ default: fetch3 }) => fetch3(...args)).catch((error) => {
  ...
  "node_modules/@supabase/storage-js/dist/main/lib/helpers.js"(exports2) {
    ...
    var resolveFetch = (customFetch) => {
      ...
      if (customFetch) {
        ...
        _fetch = (...args) => Promise.resolve(`${"@supabase/node-fetch"}`).then((s) => tslib_1.__importStar(require(s))).then(({ default: fetch3 }) => fetch3(...args));
      } else {
        _fetch = fetch;
      }
      return (...args) => _fetch(...args);
...
var require_fetch3 = __commonJS({
  "node_modules/@supabase/supabase-js/dist/main/lib/fetch.js"(exports2) {
    ...
    var node_fetch_1 = __importStar2(require_lib3());
    var resolveFetch = (customFetch) => {
      let _fetch;
      if (customFetch) {
        _fetch = customFetch;
...
async function retryWithBackoff(fn, maxRetries = 3, label = "operation") {
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      console.log(`\u{1F504} ${label} - Attempt ${attempt}/${maxRetries}`);
      ...
      console.log(`\u23F3 ${label} - Retry in ${delay / 1e3}s...`);
...
exports.handler = async (event) => {
  ...
  try {
    ...
    console.log("\u{1F680} Generating project:", description);
    ...
    console.log("   \u{1F504} Auto-retry enabled: Up to 3 attempts per operation\n");
...
\u{1F3AF} CRITICAL: This is NOT a landing page or website. Build an actual working privacy tool with:
...
${includeBackend ? "BACKEND REQUIRED: Include working APIs, encryption modules, database logic, authentication, middleware." : "Frontend utility only - still must be functional, not decorative."}
...
OUTPUT ONLY THE JSON, NO MARKDOWN FENCES.`;
    console.log("\u{1F4CB} Step 1: Generating project structure...");
    const projectStructure = await retryWithBackoff(async () => {
      const structureRes = await fetch2("https://openrouter.ai/api/v1/chat/completions", {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({
          ...
          temperature: 0.7,
          // Lower for faster, more focused output
          max_tokens: 800
          // Smaller structure = faster
        })
      });
      if (!structureRes.ok) {
    ...
    if (structureOnly) {
      console.log("\u{1F4CB} Structure-only mode: Returning project plan");
      return {
        statusCode: 200,
        headers,
        body: JSON.stringify({
    ...
    for (const fileSpec of projectStructure.files) {
      console.log(`
   \u{1F4DD} Generating ${fileSpec.path}...`);
...
\u{1F5A5}\uFE0F INTERFACE (NOT A WEBSITE):
...
OUTPUT ONLY THE CODE, NO MARKDOWN FENCES OR EXPLANATIONS.`;
        const fileRes = await fetch2("https://openrouter.ai/api/v1/chat/completions", {
          method: "POST",
          headers: {
            "Authorization": `Bearer ${OPENROUTER_API_KEY}`,
            "Content-Type": "application/json",
          ...
          body: JSON.stringify({
            ...
            max_tokens: 2500
            // Reduced for speed (still enough for good code)
          })
        });
        if (!fileRes.ok) {
    ...
    console.log("\u{1F680} Creating CodeSandbox preview...");
    const sandboxFiles = {};
    generatedFiles.forEach((file) => {
      sandboxFiles[file.path] = { content: file.content };
    });
    const sandboxRes = await fetch2("https://codesandbox.io/api/v1/sandboxes/define?json=1", {
      method: "POST",
...
//# sourceMappingURL=create-fullstack-tool.js.map
