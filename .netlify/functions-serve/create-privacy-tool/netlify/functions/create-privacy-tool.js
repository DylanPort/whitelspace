  ...
  "node_modules/@supabase/functions-js/dist/main/helper.js"(exports2) {
    ...
    var resolveFetch = (customFetch) => {
      ...
      if (customFetch) {
        ...
        _fetch = (...args) => Promise.resolve(`${"@supabase/node-fetch"}`).then((s) => __importStar2(require(s))).then(({ default: fetch2 }) => fetch2(...args));
      } else {
        _fetch = fetch;
      }
...
var require_PostgrestBuilder = __commonJS({
  "node_modules/@supabase/postgrest-js/dist/cjs/PostgrestBuilder.js"(exports2) {
    ...
    Object.defineProperty(exports2, "__esModule", { value: true });
    var tslib_1 = (init_tslib_es6(), __toCommonJS(tslib_es6_exports));
    var node_fetch_1 = tslib_1.__importDefault(require_lib2());
      ...
      constructor(endPoint, options) {
        ...
        this._resolveFetch = (customFetch) => {
          ...
          if (customFetch) {
            ...
            _fetch = (...args) => Promise.resolve(`${"@supabase/node-fetch"}`).then((s) => tslib_1.__importStar(require(s))).then(({ default: fetch2 }) => fetch2(...args)).catch((error) => {
  ...
  "node_modules/@supabase/storage-js/dist/main/lib/helpers.js"(exports2) {
    ...
    var resolveFetch = (customFetch) => {
      ...
      if (customFetch) {
        ...
        _fetch = (...args) => Promise.resolve(`${"@supabase/node-fetch"}`).then((s) => tslib_1.__importStar(require(s))).then(({ default: fetch2 }) => fetch2(...args));
      } else {
        _fetch = fetch;
      }
      return (...args) => _fetch(...args);
...
var require_fetch3 = __commonJS({
  "node_modules/@supabase/supabase-js/dist/main/lib/fetch.js"(exports2) {
    ...
    var node_fetch_1 = __importStar2(require_lib2());
    var resolveFetch = (customFetch) => {
      let _fetch;
      if (customFetch) {
        _fetch = customFetch;
...
// netlify/functions/create-privacy-tool.js
var { createClient } = require_main5();
var SUPABASE_URL = process.env.SUPABASE_URL;
var SUPABASE_KEY = process.env.SUPABASE_KEY;
var OPENAI_API_KEY = process.env.GW_OPENAI_API_KEY;
var OPENAI_MODEL = process.env.GW_OPENAI_MODEL || "gpt-4o-mini";
var VALTOWN_API_KEY = process.env.VALTOWN_API_KEY;
var VERCEL_TOKEN = process.env.VERCEL_TOKEN;
var VERCEL_TEAM_ID = process.env.VERCEL_TEAM_ID;
var DEPLOY_PROVIDER = (process.env.DEPLOY_PROVIDER || "").toLowerCase();
var CF_ACCOUNT_ID = process.env.CF_ACCOUNT_ID;
var CF_API_TOKEN = process.env.CF_API_TOKEN;
var CF_KV_NAMESPACE_ID = process.env.CF_KV_NAMESPACE_ID;
var CF_WORKER_HOST = process.env.CF_WORKER_HOST;
var OPENROUTER_API_KEY = process.env.OPENROUTER_API_KEY;
var OPENROUTER_MODEL_SPEC = process.env.OPENROUTER_MODEL_SPEC || "anthropic/claude-3.5-sonnet";
var OPENROUTER_MODEL_RENDER = process.env.OPENROUTER_MODEL_RENDER || process.env.OPENROUTER_MODEL || "anthropic/claude-3.5-sonnet";
var FORCE_OPENROUTER = String(process.env.FORCE_OPENROUTER || "").toLowerCase() === "true";
exports.handler = async (event) => {
  ...
  try {
    ...
    console.log("\u{1F916} Generating privacy tool:", description);
...
BUILD THIS AS IF IT'S A REAL CLIENT PROJECT WHERE YOU'RE SHOWING OFF YOUR UNIQUE VISION.`;
          console.log("\u{1F3A8} OpenAI GPT-4o: Starting generation...");
          ...
          console.log("\u{1F4E6} OpenAI response received, processing...");
          if (oaiResp.ok) {
...
BUILD THIS AS IF IT'S A REAL CLIENT PROJECT WHERE YOU'RE SHOWING OFF YOUR UNIQUE VISION.`;
          console.log(`\u{1F3A8} OpenRouter (${OPENROUTER_MODEL_RENDER}): Starting generation...`);
          ...
          console.log("\u{1F4E6} OpenRouter response received, processing...");
          if (orResp.ok) {
      ...
      if (!fallbackHtml) {
        let addFeature2 = function(icon, title, desc) {
          featurePresets.push({ icon, title, desc });
        };
        ...
        if (d.includes("wallet")) addFeature2("\u{1F45B}", "Seedless Onboarding", "Passwordless sign ups with passkeys");
        if (d.includes("passkey")) addFeature2("\u{1F511}", "Passkey Security", "FIDO2-based authentication");
        if (d.includes("guardian") || d.includes("recovery")) addFeature2("\u{1F6E1}\uFE0F", "Guardian Recovery", "Trusted social recovery workflow");
        if (d.includes("swap")) addFeature2("\u{1F504}", "Built-in Swap", "Token swaps with best routes");
        if (d.includes("nft")) addFeature2("\u{1F5BC}\uFE0F", "NFT Gallery", "Beautiful on-chain collectibles view");
        if (featurePresets.length === 0) {
          addFeature2("\u26A1", "Blazing Fast", "Optimized static site, instant loads");
          addFeature2("\u{1F512}", "Private by Default", "No tracking, no cookies");
          addFeature2("\u{1F3AF}", "Purpose Built", "Designed around your exact use-case");
        }
    ...
    try {
      console.log("\u2705 HTML generated length:", (fallbackHtml || "").length);
    } catch {
    }
    ...
    if (!deployedUrl && (fallbackHtml || cleanCode)) {
      try {
        console.log("\u{1F680} Deploying to CodeSandbox...");
        const htmlOut = fallbackHtml || `<!DOCTYPE html><html><head><meta charset="utf-8"/><meta name="viewport" content="width=device-width,initial-scale=1"/><script src="https://cdn.tailwindcss.com"></script><title>Tool</title></head><body class="bg-slate-950 text-slate-100 p-8"><pre class="whitespace-pre-wrap">${cleanCode.replace(/</g, "&lt;")}</pre></body></html>`;
...
Creator: ${creatorWallet || "Anonymous"}`
    ...
    if (!deployedUrl) {
      if (aiFallbackUsed && fallbackHtml) {
        ...
        const { data: toolData2, error: dbError } = await supabase.from("privacy_tools").insert({
          val_id: null,
          val_name: toolName,
          description,
          category: category || "General",
    ...
    try {
      const supabase = createClient(SUPABASE_URL, SUPABASE_KEY);
      const insertRes = await supabase.from("privacy_tools").insert({
    ...
    return {
      ...
      body: JSON.stringify({
        ...
        tool: {
          ...
          message: "\u{1F389} Your privacy tool has been created!",
          html: fallbackHtml,
...
//# sourceMappingURL=create-privacy-tool.js.map
