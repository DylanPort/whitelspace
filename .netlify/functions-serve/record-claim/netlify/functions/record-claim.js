...
// netlify/functions/record-claim.js
var { createClient } = require_main5();
var SUPABASE_URL = process.env.SUPABASE_URL || "https://avhmgbkwfwlatykotxwv.supabase.co";
...
var CLAIM_LOCK_MINUTES = 5;
var CLAIM_LOCK_MS = CLAIM_LOCK_MINUTES * 60 * 1e3;
var BLOCKED_WALLETS = /* @__PURE__ */ new Set([
  ...
  try {
    if (event.httpMethod === "POST") {
      ...
      if (BLOCKED_WALLETS.has(walletAddress)) {
        console.warn(`\u{1F6AB} Blocked wallet attempted to record claim: ${walletAddress}`);
        return {
          statusCode: 403,
          headers,
          body: JSON.stringify({
      ...
      const { data, error } = await supabase.from("claim_history").upsert({
      ...
      return {
        ...
        body: JSON.stringify({
          success: true,
          timestamp,
          nextClaimAvailable: timestamp + COOLDOWN_HOURS * 60 * 60 * 1e3
        })
      };
      ...
      if (BLOCKED_WALLETS.has(walletAddress)) {
        console.warn(`\u{1F6AB} Blocked wallet attempted to query claim history: ${walletAddress}`);
        return {
          statusCode: 403,
          headers,
          body: JSON.stringify({
      ...
      const { data, error } = await supabase.from("claim_history").select("*").eq("wallet_address", walletAddress).single();
      ...
      const cooldownMs = COOLDOWN_HOURS * 60 * 60 * 1e3;
      ...
      return {
        ...
        body: JSON.stringify({
          ...
          hoursUntilNextClaim: Math.ceil(timeUntilNextClaim / (60 * 60 * 1e3)),
          claimLock: data.claim_lock || false,
          claimLockExpires: data.claim_lock_expires_at,
          claimAmount: data.claim_amount,
          message: canClaim ? "Ready to claim" : `Please wait ${Math.ceil(timeUntilNextClaim / (60 * 60 * 1e3))} hours before claiming again`
        })
      };
    }
    return {
...
//# sourceMappingURL=record-claim.js.map
