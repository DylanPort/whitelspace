...
// netlify/functions/vote-privacy-tool.js
...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (voterWallet) {
      const { data: existingVote } = await supabase.from("tool_votes").select("*").eq("tool_id", toolId).eq("voter_wallet", voterWallet).single();
      if (existingVote) {
        if (existingVote.vote_type === voteType) {
          await supabase.from("tool_votes").delete().eq("id", existingVote.id);
          const column2 = voteType === "upvote" ? "upvotes" : "downvotes";
          ...
          await supabase.from("tool_votes").update({ vote_type: voteType }).eq("id", existingVote.id);
      ...
      await supabase.from("tool_votes").insert({
        tool_id: toolId,
    ...
    const { data: tool, error } = await supabase.from("privacy_tools").update({
...
//# sourceMappingURL=vote-privacy-tool.js.map
