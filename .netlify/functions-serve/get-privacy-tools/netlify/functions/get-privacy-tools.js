...
// netlify/functions/get-privacy-tools.js
...
exports.handler = async (event) => {
  ...
  try {
    ...
    let query = supabase.from("privacy_tools").select("*");
    if (status !== "all") {
    ...
    const { data: categoryCounts } = await supabase.from("privacy_tools").select("category").eq("status", "approved");
    const categories = {};
    if (categoryCounts) {
...
//# sourceMappingURL=get-privacy-tools.js.map
