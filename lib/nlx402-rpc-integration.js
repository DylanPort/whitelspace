...
const { Nlx402Client, Nlx402Error } = require('../NLx402-SDK/javascript/nlx402');

class WhistleNLx402Integration {
  constructor(options = {}) {
    // Initialize NLx402 client
    this.nlx402 = new Nlx402Client({
      apiKey: options.apiKey || process.env.NLX402_API_KEY,
      baseUrl: options.baseUrl || process.env.NLX402_BASE_URL || 'https://pay.thrt.ai',
    });
  ...
  async generateRPCQuote({ walletAddress, queryCount = 1, rpcMethod }) {
    try {
      ...
      // Get NLx402 quote
      const quote = await this.nlx402.getQuote({
      ...
      this.verifiedNonces.set(quote.nonce, {
        timestamp: Date.now(),
        walletAddress,
        queryCount,
        rpcMethod,
  ...
  async verifyRPCQuote({ quote, nonce, walletAddress }) {
    try {
      ...
      // Verify with NLx402
      const result = await this.nlx402.verifyQuote({ quote, nonce });
  ...
  async unlockRPCAccess({ tx, nonce, walletAddress }) {
    try {
      ...
      // Check with NLx402 for payment confirmation
      const paid = await this.nlx402.getPaidAccess({ tx, nonce });
